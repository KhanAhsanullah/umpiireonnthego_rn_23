...
export const LocationPicker = (props: any) => {
  ...
  const getLocation = (ask: boolean = true) => {
    getCurrentLocation(ask)
      .then((res: any) => {
        ...
        getGeocode(res.latitude, res.longitude)
          .then((res: any) => {
            const {lat, lng} = res.results[0].geometry.location;

            setOrigin(res.results[0]?.formatted_address);
            location[0] = {
              latitude: lat,
              longitude: lng,
  ...
  const removeSuggestion = item => {
    const sugIndex = suggestions.findIndex(
        ...
        <View
          ...
          {suggestions.map((item: any, index) => (
            <TouchableOpacity
              ...
              <Typography textType="light" style={{flex: 1}}>
                {item.address}
              </Typography>
